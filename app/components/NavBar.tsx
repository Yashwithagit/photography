...
list-style: none;
transition: 0.3s ease-in-out;
color:${(props) => props.active ? '#17cf97 ' : 'none'};
        ...
        <NavBar active={isOpen}>
          {
            NavbarData.map((navItem, index) => {
              return (
                ...
                <NavBarItem key={index} active={navItem.path === currentPath}>
                  <Link href={navItem.path} onClick={() => handleOnClickMenuItem(navItem.path)}>{navItem.title}</Link>
                </NavBarItem>
              )
            })
          }
